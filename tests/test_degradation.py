...
from auditable_mcp.session import AmcpSession, new_session_id
...
def _cap(
    *, level: Level = Level.L1, countersign: Countersign = Countersign.NONE, version: str = SPEC_VERSION
) -> AuditCapability:
    """A capability varying only the axis under test."""
    return AuditCapability(spec_version=version, level=level, attempt='request', countersign=countersign)
    # end def
...
def test_the_mandatory_posture_declines_to_serve() -> None:
    """[SEP-2133] permits refusing where an uncountersigned record has no value (§6.2)."""
    with pytest.raises(UnnegotiatedCallError) as caught:
...
def test_a_mandatory_tool_never_needs_a_fallback() -> None:
    ...
    with pytest.raises(UnnegotiatedCallError):
...
@pytest.mark.asyncio
async def test_a_degraded_session_keeps_recording_and_the_chain_is_uncountersigned() -> None:
    """The recording does not stop; the host's countersignature does, and the records say so (§5.2, §6.2)."""
    ...
    session = AmcpSession(transport, host.open_session(new_session_id()), deps=_FixedDeps())
...
def test_a_tool_that_requires_a_countersign_cannot_degrade() -> None:
    """Its own host holds no key a registry binds to a host, so every action would abort (§5.2, §6.2)."""
    requiring = _cap(countersign=Countersign.HOST)
...
def test_a_tool_that_requires_a_countersign_may_still_refuse_to_serve() -> None:
    """The coherent posture for that requirement is mandatory, and it still works (§6.2)."""
    with pytest.raises(UnnegotiatedCallError):
        transport_for(
            negotiate(None, _cap(countersign=Countersign.HOST)), negotiated=_wire(), posture=Posture.MANDATORY
        )
        # end with
    # end def
