...
CAPABILITY = AuditCapability(spec_version=SPEC_VERSION, level=Level.L1, attempt='request', witness=Witness.NONE)
WITNESSING = AuditCapability(spec_version=SPEC_VERSION, level=Level.L1, attempt='request', witness=Witness.HOST)
CONCURRENT = 5
...
class _SlowSigner:
    """A witness signer that yields to the loop, as an HSM or KMS client does."""
...
async def test_a_witnessing_host_seals_concurrent_attempts_atomically() -> None:
    """So does signing: an HSM call is the same window, and the one production hosts always have."""
    host = AuditHost('tenant-a', WITNESSING, witness_signer=_SlowSigner())
    _assert_chain_holds(await _seal_concurrently(host))
    # end def
...
async def test_concurrent_duplicates_do_not_both_pass_the_uniqueness_check() -> None:
    ...
    accepted = [response for response in responses if response.status == 'accept']
    rejected = [response for response in responses if response.status == 'reject']
    assert len(accepted) == 1, 'a duplicate attempt id was sealed more than once'
    assert all(response.reason == 'replay-detected' for response in rejected)
