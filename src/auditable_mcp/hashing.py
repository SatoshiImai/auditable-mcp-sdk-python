...
def countersignature_payload(seq: int, host_ts: str, log_id: str, previous_hash: str, record_hash: str) -> bytes:
    ...
    record-hash preimage: a record sealed with a countersignature and the same record sealed without
    one have the same `record_hash` (§5.2). `log_id` names the chain the statement is about, so a
    ...
    Args:
        ...
        log_id: The name the host gives the partition's chain.
        previous_hash: The preceding record's `record_hash`.
        record_hash: This record's hash.

    Returns:
        The RFC 8785 canonical form of the five fields, UTF-8 encoded.
    """
    payload = {
        'host_ts': host_ts,
        'log_id': log_id,
