...
[SEP-2133] carries an extension's settings object in the `extensions` member of the capabilities each
party sends at `initialize` — `ClientCapabilities` for the host, `ServerCapabilities` for the tool —
keyed by the extension identifier. These helpers put the audit capability there and take a peer's back
out. They work on any pydantic model or mapping rather than on the official MCP types, so one pair of
calls serves both sides and neither drags in the MCP package: `extensions` is an open member and the
capability models accept extras, so a declaration survives a peer whose types predate [SEP-2133].

A declaration that does not validate is not a declaration: `capability_of` returns None for it, and
§6.2 then governs the session exactly as it governs a peer that declared nothing. An operator reading
...
def declare_into(capabilities: MutableMapping[str, Any], capability: AuditCapability) -> None:
    ...
    so the binding merges the declaration in as the handshake passes. Other extensions are kept.

    Args:
        capabilities: The `capabilities` member of an `initialize` request or result, mutated in place.
