...
def sign_event(
    event: dict[str, object], key_id: str, sequence: int, private_key: Ed25519PrivateKey
) -> dict[str, object]:
    """Return `event` stamped with `key_id`, `sequence`, and a base64 detached Ed25519 signature."""
    signed = {**event, fields.KEY_ID: key_id, fields.SEQUENCE: sequence}
...
class Ed25519Signer:
    ...
    def __init__(self, key_id: str, private_key: Ed25519PrivateKey, *, start_sequence: int = 0) -> None:
        """Bind the signer to a key and the next sequence value it will emit."""
        self._key_id = key_id
    ...
    @classmethod
    def from_tool_key(cls, tool_key: ToolKey, *, start_sequence: int = 0) -> 'Ed25519Signer':
        """Build a signer from a generated tool key."""
        return cls(tool_key.key_id, tool_key.private_key, start_sequence=start_sequence)
        # end def
    ...
    async def sign(self, event: dict[str, object]) -> dict[str, object]:
        ...
        return sign_event(event, self._key_id, sequence, self._private_key)
        # end def
